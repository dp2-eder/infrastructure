...
BASE_IMG_PATH="noble-server-cloudimg-amd64.img"
...
if [ ! -f "$BASE_IMG_PATH" ]; then
    echo "No se encontró la imagen base en $BASE_IMG_PATH"
    exit 1
fi
...
# --- 4. FUNCIÓN PARA CREAR VM (MODIFICADA) ---
...
crear_vm() {
    ...
    sudo qemu-img create -f qcow2 -F qcow2 -b "$BASE_IMG_PATH" "$DISK_PATH"
